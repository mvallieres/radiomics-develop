...
imParamMR.discretisation.texture.type = {'FBN','FBNequal'}; % OPTION: Using two different types of quantization algorithms (always fixed bin number for MR for now). If "FBS" or "FBSequal" is used, imParam.reSeg.range must be defined, as userSetMinVal will be assigned to imParam.reSeg.range(1).
...
if ~isempty(imParamMR.reSeg.range)
    imParamMR.userSetMinVal = imParamMR.reSeg.range(1);
else
    imParamMR.userSetMinVal = 0; % Forcing MRI intensities to be defined at minimum 0 for FBS algorithm for all features type in case range = [] (range(1) always used first as minimum value). To use the minimum value of the ROI in a FBS algorithm, use range = [] and userSetMinVal = [] (not recommended). Normally, this value is never used since only FBN algorithm is used with MRI.
end
...
if ~isempty(imParamCT.reSeg.range)
    imParamCT.userSetMinVal = imParamCT.reSeg.range(1);
else
    imParamCT.userSetMinVal = -1000; % Forcing CT HU to be defined at minimum -1000 (no assumption on the valid range) for FBS algorithm for all features type in case range = [] (range(1) always used first as minimum value). To use the minimum value of the ROI in a FBS algorithm, use range = [] and userSetMinVal = [] (not recommended).
end
...
imParamPET.reSeg.range = [0,inf]; % OPTION: We are working with SUV maps, going from 0 to infinity. The minimal value is of special importance for FBS discretisation.
...
if ~isempty(imParamPET.reSeg.range)
    imParamPET.userSetMinVal = imParamPET.reSeg.range(1);
else
   imParamPET.userSetMinVal = 0; % Forcing SUV to be defined at minimum 0 for FBS algorithm for all features type in case range = [] (range(1) always used first as minimum value). To use the minimum value of the ROI in a FBS algorithm, use range = [] and userSetMinVal = [] (not recommended).
end
...
% FILTER PARAMETERS (considered as arbitrary intensities). No FBS algorithm should ever be used here, or otherwise the code will fail.
...
imParamFilter.discretisation.texture.type = {'FBN','FBNequal'}; % Using two different types of quantization algorithms (always fixed bin number for MR for now). If "FBS" or "FBSequal" is used, imParam.reSeg.range must be defined, as userSetMinVal will be assigned to imParam.reSeg.range(1).
